...
export default function TechStack() {
  ...
  return (
    <div
      className="min-h-screen bg-[#0f1c23] dark:bg-white text-white font-display px-6 sm:px-10 py-20"
      id="TechStack"
    >
      <div className="max-w-6xl mx-auto space-y-12">
        {/* Section Header */}
        <div className="text-center md:text-left">
          <h2 className="text-4xl sm:text-5xl font-black mb-4 text-blue-500">
            Tech Stack
          </h2>
          <p className="text-zinc-400 text-lg sm:text-xl max-w-2xl dark:text-zinc-600">
            Tools & technologies I use to build smooth and scalable digital
            experiences.
          </p>
        </div>
        ...
        <div className="grid gap-10">
          {categories.map((category, index) => (
            <div
              key={index}
              className="bg-[#1a1a1a]/40 dark:bg-zinc-50 border border-white/5 dark:border-black/5 rounded-3xl p-8 sm:p-10 shadow-2xl"
            >
              <h3 className="text-2xl font-bold mb-8 dark:text-black">
                {category.title}
              </h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 sm:gap-8">
                {category.items.map((item, i) => (
                  ...
                  >
                    ...
                    <p className="text-sm sm:text-base text-zinc-400 font-semibold group-hover:text-white dark:group-hover:text-black transition-colors dark:text-zinc-600">
                      {item.name}
                    </p>
                  </div>
                ))}
