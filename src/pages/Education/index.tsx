...
export default function EducationSection() {
  ...
  return (
    <div className="min-h-screen bg-[#0f1c23] text-white font-display flex flex-col items-center  px-4 sm:px-10 lg:px-32">
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-6 py-6 relative">
          {educationData.map((edu, index) => (
            <div key={index} className="contents">
              ...
              {/* Education Card */}
              <div className="pb-12">
                <Card className="bg-[#1a1a1a]/20 border border-zinc-800 hover:border-primary/50 transition-all duration-300 hover:-translate-y-1 p-6 rounded-xl">
                  <p className="text-xl font-semibold text-white">{edu.title}</p>
                  <p className="text-zinc-400 text-base">{edu.institution}</p>
                  <p className="text-zinc-500 text-sm mt-1">{edu.period}</p>

                  <div className="mt-4">
                    <h3 className="text-zinc-300 font-medium text-sm mb-2">
                      {edu.sectionTitle}
                    </h3>
